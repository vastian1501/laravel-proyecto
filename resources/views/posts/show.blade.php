            ...
            <div class="md:w-1/2 p-5">
                ...
                <div class="py-4 flex items-center justify-between gap-4">
                    @auth
                        ...
                        <p class="font-bold my-4 mx-2">{{ $post->likes()->count() }} <span class="font-normal">likes</span></p>
                    </div>
                    {{-- Solo puede eliminar la publicacion si esta autenticado y si es el creador del post --}}
                    @auth
                        <div>
                            @if ($post->user_id === auth()->user()->id)
                                <form action="{{ route('post.delete', $post) }}" method="POST" class="self-end">
                                    {{-- Mehotd spoofing, el navegador solo acepta POST Y GET --}}
                                    @method('DELETE')
                                    @csrf
                                    <input type="submit" value="Eliminar Publicacion"
                                        class="bg-red-400 hover:bg-red-500 p-2 rounded text-white font-semibold cursor-pointer">
                                </form>
                            @endif
                        </div>
                    @endauth
